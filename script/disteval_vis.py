...
def render_main_diagonal(_L, _axis, _ss_dict, _cmap):
    ...
    for i in range(_L - 1):
        ...
        if i != 0:
            if i + 1 in _ss_dict and _ss_dict[i + 1] == 'H':
                _color = 'red'
            elif i + 1 in _ss_dict and _ss_dict[i + 1] == 'E':
                _color = 'green'
            elif i + 1 in _ss_dict and _ss_dict[i + 1] == 'C':
                _color = "#eeeeee"
...
def dmap2chordimage(ND=None, chord_file=None, ss={}, cmap_for_chord='jet'):
    ...
    for i in range(L):
        ...
        e1 = matplotlib.patches.Arc(xy=(.5, .5), width=1, height=1, linewidth=12,
                                    angle=i * size_of_each_protein_in_chord_map,
    ...
    for i in range(L):
        ...
        if i == 0: mycolor = cmap(float(i) / L)
        e1 = matplotlib.patches.Arc(xy=(.5, .5), width=1, height=1, linewidth=7,
                                    angle=i * size_of_each_protein_in_chord_map,
                                    theta2=size_of_each_protein_in_chord_map, color=mycolor)
...
def do_visualization(script_args, native_basename, basename, ND, D, NC, C, pearson_8=0, pearson_20=0):
    ...
    if script_args.color_scheme == 1:
        _col_scheme = "standard"
    elif script_args.color_scheme == 2:
        _col_scheme = "semantic"
    elif script_args.color_scheme == 3:
        _col_scheme = "uniform"
...
def calc_dist_errors(P, Y, dist_thres):
    errors = {}
    errors['mae'] = np.nan
    errors['mse'] = np.nan
    errors['rmse'] = np.nan
    ...
    for p in range(len(Y)):
        for q in range(len(Y)):
            ...
            true_dict[(p, q)] = Y[p, q]
            true_list.append(Y[p, q])
            pred_dict[(p, q)] = P[p, q]
            pred_list.append(P[p, q])

    errors['mae'] = round(metrics.mean_absolute_error(true_list, pred_list), 2)
    errors['mse'] = round(metrics.mean_squared_error(true_list, pred_list), 2)
    errors['rmse'] = round(sqrt(errors['mse']), 2)
    errors['pearsonr'] = round(pearsonr(true_list, pred_list)[0], 2)
    return errors