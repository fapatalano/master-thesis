...
p.add_argument('-dataAB', '--inputAB', '-k', required=True, help='Input trRossetta NPZ file(s) for the mergedsequence',
               nargs="+")
...
p.add_argument('-fast', '--fast', '-f', required=False, default="False",
               help='Do not merge NPZ files position by position', action='store_true')
...
np.savez_compressed(ns.output, dist=new_rst['dist'], omega=new_rst['omega'], theta=new_rst['theta'],
                    phi=new_rst['phi'])  # , rep=new_rst['rep'])