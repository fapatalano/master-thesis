...
def capri_class(fnat, iRMS, LRMS, capri_peptide=False):
    if capri_peptide:
        ...
        if (fnat < 0.2 or (LRMS > 5.0 and iRMS > 2.0)):
            return 'Incorrect'
        elif ((fnat >= 0.2 and fnat < 0.5) and (LRMS <= 5.0 or iRMS <= 2.0) or (
                fnat >= 0.5 and LRMS > 2.0 and iRMS > 1.0)):
            return 'Acceptable'
        elif ((fnat >= 0.5 and fnat < 0.8) and (LRMS <= 2.0 or iRMS <= 1.0) or (
                fnat >= 0.8 and LRMS > 1.0 and iRMS > 0.5)):
            return 'Medium'
        ...
        if (fnat < 0.1 or (LRMS > 10.0 and iRMS > 4.0)):
            return 'Incorrect'
        elif ((fnat >= 0.1 and fnat < 0.3) and (LRMS <= 10.0 or iRMS <= 4.0) or (
                fnat >= 0.3 and LRMS > 5.0 and iRMS > 2.0)):
            return 'Acceptable'
        elif ((fnat >= 0.3 and fnat < 0.5) and (LRMS <= 5.0 or iRMS <= 2.0) or (
...
def calc_DockQ(model, native, use_CA_only=False, capri_peptide=False):
    ...
    (fnat_bb, nat_correct_bb, nat_total_bb, fnonnat_bb, nonnat_count_bb, model_total_bb, interface) = parse_fnat(
        inter_out)
    ...
    # print (ref_atoms,sample_atoms)
    # assert len(ref_atoms)!=0, "length of native is zero"
    # assert len(sample_atoms)!=0, "length of model is zero"
    if (len(ref_atoms) == 0 or len(sample_atoms) == 0):
        # print( "length of native is zero")
        info = {}
        info['DockQ'] = 0
        info['irms'] = 0
        info['Lrms'] = 0
    ...
    assert len(chain_ref[receptor_chain]) == len(
        chain_sample[receptor_chain]), "Different number of atoms in native and model receptor (chain %c) %d %d\n" % (
    ...
    assert len(chain_ref[ligand_chain]) == len(
        chain_sample[ligand_chain]), "Different number of atoms in native and model ligand (chain %c) %d %d\n" % (
    ligand_chain, len(chain_ref[ligand_chain]), len(chain_sample[ligand_chain]))
...
def main():
        ...
        if args.perm1 or args.perm2:
            ...
            for g1 in combos1:
                for g2 in combos2:
                    ...
                    if not args.quiet:
                        print(str(pe) + '/' + str(pe_tot) + ' ' + ''.join(g1) + ' -> ' + ''.join(g2) + ' ' + str(
                            test_dict['DockQ']))
                    if (test_dict['DockQ'] > best_DockQ):
                        ...
                        best_g1 = g1
                        best_g2 = g2
                        best_info = 'Best score ( ' + str(
                            best_DockQ) + ' ) found for model -> native, chain1:' + ''.join(best_g1) + ' -> ' + ''.join(
                            nat_group1) + ' chain2:' + ''.join(best_g2) + ' -> ' + ''.join(nat_group2)
    ...
    if (args.short):
        if capri_peptide:
            print(("DockQ-capri_peptide %.3f Fnat %.3f iRMS %.3f LRMS %.3f Fnonnat %.3f %s %s %s" % (
            DockQ, fnat, irms, Lrms, fnonnat, model_in, native_in, best_info)))
        else:
            print(("DockQ %.3f Fnat %.3f iRMS %.3f LRMS %.3f Fnonnat %.3f %s %s %s" % (
            DockQ, fnat, irms, Lrms, fnonnat, model_in, native_in, best_info)))
        ...
        if len(best_info):
            print(best_info)
        print('Number of equivalent residues in chain ' + info['chain1'] + ' ' + str(info['len1']) + ' (' + info[
            'class1'] + ')')
        print('Number of equivalent residues in chain ' + info['chain2'] + ' ' + str(info['len2']) + ' (' + info[
            'class2'] + ')')
        print(("Fnat %.3f %d correct of %d native contacts" % (info['fnat'], info['nat_correct'], info['nat_total'])))
        print(("Fnonnat %.3f %d non-native of %d model contacts" % (
