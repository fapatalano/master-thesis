...
def add_intrachain_rst(npz, rst, tmpdir, params, minprob=0.5, LB=1, UB=15, D=20, WD=1000, WB=50, allcontacts=False):
    ...
    for i in range(params["seqlen1"]):
        for j in range(params["seqlen1"] + 1, params["seqlen2"] + params["seqlen1"]):
            # We should limit ourself to constrains that have a probability
            if prob[i, j] > minprob:
                ...
                # Zero in the range of x0 - tol to x0 + tol. Harmonic with width parameter sd outside that range.
                # Basically, a HARMONIC potential (see above) split at x0 with a 2*tol length region of zero
                # inserted. BOUNDED lb ub sd rswitch tag rst_line = 'AtomPair %s %d %s %d BOUNDED  %.5f %.5f %.5f
                # %.5f %s '%('CB',i+1,'CB',j+1,0,UB*2,D,0.5,"Bounded")
    ...
    for i in range(params["seqlen1"]):
        for j in range(params["seqlen1"] + 1, params["seqlen2"] + params["seqlen1"]):
            if list_i[i] > 0 and list_j[j] > 0:
                ...
                # Basically, a HARMONIC potential (see above) split at x0 with a 2*tol length region of zero
                # inserted. BOUNDED lb ub sd rswitch tag rst_line = 'AtomPair %s %d %s %d BOUNDED  %.5f %.5f %.5f
                # %.5f %s '%('CB',i+1,'CB',j+1,0,UB*2,D,0.5,"Bounded") Using default from
                rst_line = 'AtomPair %s %d %s %d FLAT_HARMONIC  %.5f %.5f %.5f' % ('CB', i + 1, 'CB', j + 1, 0, WD, WB)
                # print (rst_line)
                rst['intrachain'].append([i, j, 1.0, rst_line])
    ...
    for i in range(params["seqlen1"]):
        for j in range(params["seqlen1"] + 1, params["seqlen2"] + params["seqlen1"]):
            ...
            if (prob[i, j] < minprob):  # We actually ignore interacting pairs.
                ...
                rst_line = 'AtomPair %s %d %s %d FLAT_HARMONIC  %.5f %.5f %.5f' % (
                    'CB', i + 1, 'CB', j + 1, WD, 1, WD - WB)
                rst['intrachain'].append([i, j, 1.0, rst_line])
    print("Flat harmonic restraints:  %d" % (len(rst['intrachain'])))
...
def gen_rst(npz, tmpdir, params):
    ...
    '''prob = np.sum(dist[:,:,5:], axis=-1) i,j = np.where(prob<PREP) prob = prob[i,j] for a,b,p in zip(i,j,
    prob): if b>a: name=tmpdir.name+"/%d.%d_rep.txt"%(a+1,b+1) rst_line = 'AtomPair %s %d %s %d SCALARWEIGHTEDFUNC 
    %.2f SUMFUNC 2 CONSTANTFUNC 0.5 SIGMOID %.3f %.3f\n'%('CB',a+1,'CB',b+1,-0.5,SIGD,SIGM) rst['rep'].append([a,b,p,
    rst_line]) print("rep restraints:   %d"%(len(rst['rep']))) '''
    ...
    for a, b, p in zip(i, j, prob):
        if b != a:
            ...
            rst_line = 'Dihedral N %d CA %d CB %d CB %d SPLINE TAG %s 1.0 %.3f %.5f' % (
                a + 1, a + 1, a + 1, b + 1, name, 1.0, ASTEP)
...
def add_rst_chain2(pose, rst, sep1, sep2, params, nogly=False):
    ...
    if nogly:
        array += [line for a, b, p, line in rst['intrachain'] if
                  seq[b] != 'G' and seq[a] != 'G' and seq[b] != 'G' and p >= pcut]
        if inter:
            array += [line for a, b, p, line in rst['dist'] if
                      sep1 <= abs(a - b) < sep2 and seq[a] != 'G' and seq[b] != 'G' and p >= pcut]
            if params['USE_ORIENT']:
                array += [line for a, b, p, line in rst['omega'] if
                          sep1 <= abs(a - b) < sep2 and seq[a] != 'G' and seq[
                              b] != 'G' and p >= pcut + 0.5]  # 0.5
                array += [line for a, b, p, line in rst['theta'] if
                          sep1 <= abs(a - b) < sep2 and seq[a] != 'G' and seq[
                              b] != 'G' and p >= pcut + 0.5]  # 0.5
                array += [line for a, b, p, line in rst['phi'] if
                          sep1 <= abs(a - b) < sep2 and seq[a] != 'G' and seq[
                              b] != 'G' and p >= pcut + 0.6]  # 0.6
        else:
            array += [line for a, b, p, line in rst['dist'] if
                      sep1 <= abs(a - b) < sep2 and seq[a] != 'G' and seq[
                          b] != 'G' and p >= pcut and np.sign(a - seqlen) == np.sign(b - seqlen)]  # 0.5
            if params['USE_ORIENT']:
                array += [line for a, b, p, line in rst['omega'] if
                          sep1 <= abs(a - b) < sep2 and seq[a] != 'G' and seq[
                              b] != 'G' and p >= pcut + 0.5 and np.sign(a - seqlen) == np.sign(b - seqlen)]  # 0.5
                array += [line for a, b, p, line in rst['theta'] if
                          sep1 <= abs(a - b) < sep2 and seq[a] != 'G' and seq[
                              b] != 'G' and p >= pcut + 0.5 and np.sign(a - seqlen) == np.sign(b - seqlen)]  # 0.5
                array += [line for a, b, p, line in rst['phi'] if
                          sep1 <= abs(a - b) < sep2 and seq[a] != 'G' and seq[
                              b] != 'G' and p >= pcut + 0.6 and np.sign(a - seqlen) == np.sign(b - seqlen)]  # 0.6
    else:
        array += [line for a, b, p, line in rst['intrachain'] if p >= pcut]
        if inter:
            array += [line for a, b, p, line in rst['dist'] if sep1 <= abs(a - b) < sep2 and p >= pcut]
            if params['USE_ORIENT']:
                array += [line for a, b, p, line in rst['omega'] if
                          sep1 <= abs(a - b) < sep2 and p >= pcut + 0.5]
                array += [line for a, b, p, line in rst['theta'] if
                          sep1 <= abs(a - b) < sep2 and p >= pcut + 0.5]
                array += [line for a, b, p, line in rst['phi'] if
                          sep1 <= abs(a - b) < sep2 and p >= pcut + 0.6]  # 0.6
        else:
            array += [line for a, b, p, line in rst['dist'] if
                      sep1 <= abs(a - b) < sep2 and p >= pcut and np.sign(a - seqlen) == np.sign(
                          b - seqlen)]
            if params['USE_ORIENT']:
                array += [line for a, b, p, line in rst['omega'] if
                          sep1 <= abs(a - b) < sep2 and p >= pcut + 0.5 and np.sign(
                              a - seqlen) == np.sign(b - seqlen)]
                array += [line for a, b, p, line in rst['theta'] if
                          sep1 <= abs(a - b) < sep2 and p >= pcut + 0.5 and np.sign(
                              a - seqlen) == np.sign(b - seqlen)]
                array += [line for a, b, p, line in rst['phi'] if
                          sep1 <= abs(a - b) < sep2 and p >= pcut + 0.6 and np.sign(
                              a - seqlen) == np.sign(b - seqlen)]  # 0.6
...
def add_intra_rst(pose, rst, sep1, sep2, params, nogly=False):
    ...
    if nogly:
        ...
        array += [line for a, b, p, line in rst['intrachain'] if p >= pcut]
        array += [line for a, b, p, line in rst['dist'] if
                  sep1 <= abs(a - b) < sep2 and p >= pcut and np.sign(a - seqlen) != np.sign(b - seqlen)]
        if params['USE_ORIENT']:
            array += [line for a, b, p, line in rst['omega'] if
                      sep1 <= abs(a - b) < sep2 and p >= pcut + 0.5 and np.sign(a - seqlen) != np.sign(
                          b - seqlen)]
            array += [line for a, b, p, line in rst['theta'] if
                      sep1 <= abs(a - b) < sep2 and p >= pcut + 0.5 and np.sign(a - seqlen) != np.sign(
                          b - seqlen)]
            array += [line for a, b, p, line in rst['phi'] if
                      sep1 <= abs(a - b) < sep2 and p >= pcut + 0.6 and np.sign(a - seqlen) != np.sign(
                          b - seqlen)]  # 0.6
