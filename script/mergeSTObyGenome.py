...
p.add_argument('-seq', '--sepseq', '-s', required=False, help='sequence file to identify domain baorders (20 Gly)',
...
# First we should always use sequence 1 in both files...

sequences = []

sequences += [
    SeqRecord(seq=seqA.seq + sepseq + seqB.seq, id=seqA.name + " " + seqB.name, name=seqA.name + " " + seqB.name,
              description=seqA.name + " " + seqB.name, dbxrefs=[])]
...
for key in dataA.keys():
    if ((key in dataB.keys()) and (dataA[key].id != dataB[key].id)):
        sequences += [
            SeqRecord(seq=dataA[key].seq + sepseq + dataB[key].seq, id=key, name=key, description=key, dbxrefs=[])]
