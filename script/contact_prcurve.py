...
if __name__ == "__main__":
    ...
    p.add_argument('-c', required=True,
                   help='trRosetta file with .npz format (folder path only if code list is specified)')
    p.add_argument('-s1', required=True,
                   help='structure file 1 with .pdb format (folder path only if code list is specified)')
    p.add_argument('-s2', required=True,
                   help='structure file 2 with .pdb format (folder path only if code list is specified)')
    ns = p.parse_args()
    ...
    with np.load(npz) as npz_file:
        pred_cmap = npz_file['dist'][:sep, sep:, :]
