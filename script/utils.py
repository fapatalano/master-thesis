...
def parse_a3m(filename):
    ...
    msa = np.array([list(s) for s in seqs if s!=''], dtype='|S1').view(np.uint8)
...
def Conv2d(features, w, b, d=1):
    x = tf.nn.conv2d(features, tf.constant(w), strides=[1, 1, 1, 1], padding="SAME",
                     dilations=[1, d, d, 1]) + tf.constant(b)[None, None, None, :]
    return x
