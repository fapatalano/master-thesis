...
p.add_argument('-data', '--input', '-i', required=True, help='Input trRosetta NPZ file')
p.add_argument('-dataB', '--inputB', '-j', required=False,
               help='Input second trRosetta NPZ file (for instance for reversed order merged files)')
p.add_argument('-pdb', '--pdb', '-p', required=False, help='Pdb file for analysis of distances')
p.add_argument('-dom', '--domain', '-d', required=False, help='positions of domain borders', nargs='+')
p.add_argument('-seq', '--sequence', '-s', required=False,
               help='sequence file of complete sequence to identify domain borders')
p.add_argument('-first', '--firstsequence', '-f', required=False,
               help='sequence file of first sequence to identify domain borders')
p.add_argument("--sepseq", "-sep", "-S", required=False, help='Separation sequence between protein in MSA',
               default="GGGGGGGGGGGGGGGGGGGG")
...
if (ns.pdb):
    for x in range(0, 6):
        ...
        shortMCC[x] = (shortTP[x] * shortTN[x] - shortFP[x] * shortFN[x]) / np.sqrt(
            (shortTP[x] + shortFP[x]) * (shortTP[x] + shortFN[x]) * (shortTN[x] + shortFP[x]) * (
                        shortTN[x] + shortFN[x]) + 1.e-20)
        medMCC[x] = (medTP[x] * medTN[x] - medFP[x] * medFN[x]) / np.sqrt(
            (medTP[x] + medFP[x]) * (medTP[x] + medFN[x]) * (medTN[x] + medFP[x]) * (medTN[x] + medFN[x]) + 1.e-20)
        longMCC[x] = (longTP[x] * longTN[x] - longFP[x] * longFN[x]) / np.sqrt(
            (longTP[x] + longFP[x]) * (longTP[x] + longFN[x]) * (longTN[x] + longFP[x]) * (
                        longTN[x] + longFN[x]) + 1.e-20)
